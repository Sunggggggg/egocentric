...
class Renderer():
    def __init__(self, width, height, focal_length, device, faces=None, 
        ...
        self.lights = PointLights(device=device, location=[[0.0, 0.0, 5.0]])
    ...
    def set_ground(self, length, center_x, center_z):
        device = self.device
        v, f, vc, fc = map(torch.from_numpy, checkerboard_geometry(length=length, c1=center_x, c2=center_z, up="z"))
...
def get_global_cameras(verts, device, distance=5, position=(0.0, 0.0, 10.0)):
    positions = torch.tensor([position], device=verts.device).repeat(len(verts), 1)
    ...
    rotation = look_at_rotation(positions, targets, up=((0, 0, 1),), device=device).mT
