...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, config: HeadNetConfig):
        super().__init__()
        self.config = config
    ...
    def forward(
        self,
        x: Tensor,
        attn_mask: Union[Tensor, None] = None,
    ) -> Tensor:
    ...
    def _sattn(self, x: Tensor, attn_mask: Union[Tensor, None]) -> Tensor:
        config = self.config
...
class HeadNet(nn.Module):
    ...
    def __init__(self, config: HeadNetConfig):
        ...
        self.input_projection_rot = nn.Linear(6, config.d_latent // 2)
        self.input_projection_trans = nn.Linear(3, config.d_latent // 2)

        self.encoder_layers = nn.ModuleList(
            [TransformerBlock(config) for _ in range(config.num_encoder_layers)]
        )

        self.output_norm = nn.LayerNorm(config.d_latent)

    def forward(
        self,
        head_pose_trajectory: Tensor,
        mask: Union[Tensor, None] = None,
    ) -> tuple[Tensor, Tensor]:

        x_rot = self.input_projection_rot(head_pose_trajectory[..., :6])
        x_trans = self.input_projection_trans(head_pose_trajectory[..., 6:])
        x = torch.cat((x_rot, x_trans), dim=-1)
        
        # Attemtion mask?
        for layer in self.encoder_layers:
            x = layer(x, attn_mask=mask)

        encoder_output = self.output_norm(x)
