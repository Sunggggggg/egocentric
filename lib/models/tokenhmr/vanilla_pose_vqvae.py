...
class PoseSPEncoderV1(nn.Module):
    def __init__(self,
        ...
        if self.add_noise:
            from .skeleton import get_smplx_body_parts
...
class PoseSPDecoderV1(nn.Module):
    def __init__(self,
                 rot_type='rotmat',         # rot6d 
                 output_emb_width = 512,    # 256
                 down_t = 1,                # 1
                 width = 512,               # 512
                 depth = 2,                 # 2
                 token_size_div = 1,        # 4
...
class DecodeTokens(nn.Module):
    ...
    def forward(self, logits):
        decode_feat = self.quantizer.dequantize_logits(logits)  # [BT, 160, 2048] @ [2048, 256]
        pose_out = self.decoder(decode_feat.permute(0,2,1))     # [BT, 160, 256]
