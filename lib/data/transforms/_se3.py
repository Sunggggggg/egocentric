...
class SE3(_base.SEBase):
    ...
    @classmethod
    def from_rotation_and_translation(cls, rotation: SO3, translation: Tensor) -> SE3:
    ...
    @classmethod
    def identity(cls, device: torch.device | str, dtype: torch.dtype) -> SE3:
        return SE3(
    ...
    def as_matrix(self) -> Tensor:
        ...
        t = self.translation().unsqueeze(-1)
        dims = R.shape[:-2]
        bottom = (
    ...
    def adjoint(self) -> Tensor:
        ...
        return torch.cat(
            [
                ...
                torch.cat([torch.zeros((*dims, 3, 3), device=R.device, dtype=R.dtype), R], dim=-1),
            ],
            dim=-2,
        )