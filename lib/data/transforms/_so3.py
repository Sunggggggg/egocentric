...
class SO3(_base.SOBase):
    ...
    @staticmethod
    def from_rpy_radians(roll: Tensor, pitch: Tensor, yaw: Tensor) -> SO3:
        return (
    ...
    @classmethod
    def identity(cls, device: torch.device | str, dtype: torch.dtype) -> SO3:
    ...
    def as_matrix(self) -> Tensor:
        ...
        qvec = self.wxyz * torch.sqrt(2.0 / norm_sq)
        Q = torch.einsum("...i,...j->...ij", qvec, qvec)
        return torch.stack(
            [
                1.0 - Q[..., 2, 2] - Q[..., 3, 3],
                Q[..., 1, 2] - Q[..., 3, 0],
    ...
    def log(self) -> Tensor:
        ...
        norm_sq = torch.square(xyz).sum(dim=-1, keepdim=True)
