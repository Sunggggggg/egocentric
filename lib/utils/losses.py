...
class PoseReConsLoss(nn.Module):
    def __init__(self, loss_config:LossConfig) :
        ...
        self.mesh_loss = self.get_loss(loss_config.MESH_LOSS, 'mesh')
        self.pose_loss = self.get_loss(loss_config.POSE_LOSS, 'pose')
        self.jnt_loss = self.get_loss(loss_config.JNT_LOSS, 'jnt')

        if loss_config.VALID :
    ...
    def calculate_vertex_weights(self):
        ...
        body_model = eval(f'{self.smpl_type.upper()}')(_C.SMPL.SMPLH_MODEL_PATH, num_betas=10, ext='pkl')
        mesh = body_model()
