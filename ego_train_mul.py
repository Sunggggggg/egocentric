...
from lib.utils.pose_visualize import visualize_mesh, motion_contact_image_two_person
from lib.vis.renderer import Renderer
from torch.nn.parallel import DistributedDataParallel as DDP
import torch.distributed as dist

def setup_ddp():
    # torchrun --nproc_per_node=2 ego_train_mul.py
...
train_loader, val_loader = get_loader(HDF5_PATH, FILE_LIST_PATH, SUBSEQ_LEN, MUL_GPU)
...
smplh = SMPLH(**smpl_cfg).to(DEVICE)
...
for epoch in range(10000) :
    for batch in tqdm(train_loader) :
        ...
        offset = pred_jnts[:, :, [0]]                # [B, T, 1, 3]
        pred_jnts = (pred_jnts - offset)[:,:,1:]     # [B, T, 21, 3]
        ...
        gt_jnts = gt_jnts - gt_transl.unsqueeze(-2)                         # [B, T, 21, 3]
        
        motion_contact_image_two_person(pred_jnts[0, 0], pred_contact[0, 0], gt_jnts[0, 0], gt_contact[0, 0])
        ...
        if DEVICE == "cuda:0" :
            ...
            if nb_iter % _C.TRAIN.VAL_ITER  == 0:
                best_scores = eval_pose_hmr(val_loader, model, DEVICE, writer, nb_iter, save_dir, best_scores)
            ...
            if nb_iter % _C.TRAIN.PRINT_ITER  == 0:
                with torch.no_grad():
                    ...
                    batch['body_vertices'] = smpl_output.vertices.reshape(T, B, -1, 3)
                    output['pred_body_vertices'] = output['pred_verts'].reshape(T, B, -1, 3)
                    rend_img = visualize_mesh(batch, output, save_dir, nb_iter, renderer)
                    cv2.imwrite('test.jpg', rend_img)
                writer.add_image(f'rendered.jpg', rend_img.transpose(2, 0, 1), nb_iter)
